            ...
            >
              <a href="#contact-form">
                <MyButton
                  variant="primary"
                  size="large"
                  className="px-10 py-6 bg-[#FFD95A] hover:bg-[#FFD95A]/90 text-[#0D0D1A] text-lg font-bold rounded-full shadow-lg"
                  style={{ boxShadow: "0 0 30px rgba(255, 215, 0, 0.4)" }}
                  icon={<Send className="h-5 w-5" />}
                  iconPosition="left"
                >
                  {t('cta.schedule', { defaultMessage: 'Đặt lịch tư vấn' })}
                </MyButton>
              </a>
            </motion.div>
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
              <Link href={`/event/${DEMO_EVENT_ID}`}>
                <MyButton
                  variant="outline"
                  size="large"
                  className="px-10 py-6 border border-[#FFD95A] text-[#FFD95A] hover:bg-[#FFD95A]/10 text-lg font-semibold rounded-full backdrop-blur-sm"
                  icon={<Play className="h-5 w-5" />}
                  iconPosition="left"
                >
                  {t('cta.demo', { defaultMessage: 'Xem demo trực tiếp' })}
                </MyButton>
              </Link>
            </motion.div>
          </div>

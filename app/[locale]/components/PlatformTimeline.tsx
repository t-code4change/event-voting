...
import { PLATFORM_TIMELINE } from "@/constants/home.constants"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-5 gap-6 relative">
            ...
            {PLATFORM_TIMELINE.map((item, index) => {
              // Map timeline index to translation key
              const timelineKeys = ['checkin', 'welcome', 'voting', 'luckyDraw', 'analytics']
              const timelineKey = timelineKeys[index]
                ...
                >
                  <Card className="relative overflow-hidden border-2 border-[#FFD700]/20 bg-[#0D0D1A] hover:border-[#FFD700] transition-all duration-300 group">
                    <CardContent className="p-6 text-center space-y-4">
                      ...
                      <h3 className="text-lg font-semibold text-white">{t(`${timelineKey}.title`)}</h3>
                      <p className="text-gray-400 text-sm leading-relaxed">
                        {t(`${timelineKey}.description`)}
                      </p>
                    </CardContent>
                  </Card>
